# Create a raw transaction that can be spent in 2 weeks time, assuming the current block is 25

# Amount of 20,000,000 satoshis to this address: 2MvLcssW49n9atmksjwg2ZCMsEMsoj3pzUP 
# Use the UTXOs from the transaction below
# transaction=""

tx=""
recipient="2MvLcssW49n9atmksjwg2ZCMsEMsoj3pzUP"
sats=20000000
btc=$(echo "scale=8; $sats/100000000" | bc)
decodedtx=$(bitcoin-cli -regtest decoderawtransaction "$tx")

locktime= $((25 + 2 * 7 * 24 * 60 / 10))

UTXO_txid=$(echo "$decodedtx" | jq -r '.txid')

UTXO_vout1=$(echo "$decodedtx" | jq -r '.vout[0].n')
UTXO_vout2=$(echo "$decodedtx" | jq -r '.vout[1].n')

bitcoin-cli -regtest createrawtransaction '''[{"txid":"'"$UTXO_txid"'","vout":'"$UTXO_vout1"',"sequence":4294967294},{"txid":"'"$UTXO_txid"'","vout":'"$UTXO_vout2"',"sequence":4294967294}]''' '''{"'"$recipient"'":'"$btc"'}''' "$locktime"